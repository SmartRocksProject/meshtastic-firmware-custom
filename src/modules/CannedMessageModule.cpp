...
void CannedMessageModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    if (cannedMessageModule->runState == CANNED_MESSAGE_RUN_STATE_SENDING_ACTIVE) {
        ...
        sprintf(buffer, "%d left", meshtastic_Constants_DATA_PAYLOAD_LEN - this->freetext.length());
...
{
    if (!loadProto(cannedMessagesConfigFile, meshtastic_CannedMessageModuleConfig_size, sizeof(cannedMessagesConfigFile),
