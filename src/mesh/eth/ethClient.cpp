...
bool initEthernet()
{
    if (config.network.eth_enabled) {
        ...
        if (config.network.address_mode == meshtastic_Config_NetworkConfig_EthMode_DHCP) {
            DEBUG_MSG("starting Ethernet DHCP\n");
            status = Ethernet.begin(mac);
        } else if (config.network.address_mode == meshtastic_Config_NetworkConfig_EthMode_STATIC) {
